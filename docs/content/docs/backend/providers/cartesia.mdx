...
`@usevoice/cartesia` exposes a `CartesiaTtsStreamer` that converts agent responses into PCM16 audio.
...
```bash
bun add @usevoice/cartesia @cartesia/cartesia-js
```

## Usage

```ts
import { CartesiaTtsStreamer } from '@usevoice/cartesia';
