...
`@usevoiceai/deepgram` implements the `TranscriptionProvider` contract using Deepgram's Live Listen API.
...
```bash
bun add @usevoiceai/deepgram @deepgram/sdk
```
...
The rest of the API surface stays the same—the returned `TranscriptionStream` exposes `send`,
`finish`, and `abort`.
