...
then calls `stream.release()` so the controller knows it can tear down the stream reference only
after playback drains.

## 5. Explore more examples

- `bun --filter @usevoiceai/example-vue run dev` for the Vue demo.
