...
| Package                | Description                                                                                                          |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `@usevoiceai/core`     | Framework-agnostic socket client, recorder, controller, and state store.                                             |
| `@usevoiceai/react`    | `useVoiceCommand` hook that binds React apps to the core controller.                                                 |
| `@usevoiceai/vue`      | Vue composable equivalent to the React hook.                                                                         |
| `@usevoiceai/server`   | Runtime-agnostic voice session manager, adapters, and STT/TTS/agent provider scaffolding.                            |
| `@usevoiceai/deepgram` | Deepgram transcription provider that implements the `TranscriptionProvider` interface via a supplied `createClient`. |
| `@usevoiceai/cartesia` | Cartesia TTS streamer that implements the `TtsStreamer` interface with a supplied Cartesia client.                   |

Examples live under `examples/*` so we can test React and Vue integrations against the same API surface.

### Provider Packages

`@usevoiceai/deepgram` and `@usevoiceai/cartesia` are thin helpers that turn vendor SDK clients into the interfaces expected by `@usevoiceai/server`. They intentionally do **not** bundle the vendor SDKs. Install the upstream SDK in your app (e.g. `@deepgram/sdk`, `@cartesia/cartesia-js`) and supply a `clientFactory` when constructing the provider:

```ts
import { createClient as createDeepgramClient } from "@deepgram/sdk";
import { DeepgramTranscriptionProvider } from "@usevoiceai/deepgram";
...
- React: `bun --filter @usevoiceai/example-react run dev`
- Vue: `bun --filter @usevoiceai/example-vue run dev`
...
4. Start the React demo (`bun --filter @usevoiceai/example-react run dev`) and verify the status/progress updates round-trip through the worker.
...
1. Replace the mock providers with production integrations by instantiating `@usevoiceai/deepgram` / `@usevoiceai/cartesia` (or custom providers) inside your backend.
